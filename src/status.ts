...
export type TimedSendState = "waiting" | "released" | "open" | "error" | "disabled";
...
export async function writeStatus(path: string, status: TimedSendStatus): Promise<void> {
...
async function writeStatusFile(path: string, status: TimedSendStatus): Promise<void> {
...
export async function readStatus(path: string): Promise<TimedSendStatus | undefined> {
  try {
