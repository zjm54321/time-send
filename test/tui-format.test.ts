...
import { formatStatusLine, TimedSendSidebar, timedSendTui } from "../src/tui.js";
...
describe("tui formatting", () => {
  ...
  test("tui plugin registers sidebar content and plugin-owned commands", async () => {
    ...
    expect(commands.flatMap((command) => (command.slash === undefined ? [] : [command.slash.name]))).toEqual([
      "timed-send-status",
      "time-send-now",
    ]);
  ...
  test("time-send-now releases a waiting status through the shared status file", async () => {
    ...
    const sendNow = commands.find((command) => command.slash?.name === "time-send-now");
  ...
  test("display flags disable sidebar only when both legacy locations are disabled", async () => {
    ...
    await timedSendTui(api, {
      readText: async () => '{"start":"01:30","end":"09:30","display":{"promptRight":false,"appBottom":true}}',
      readStatus: async () => waiting,
      now: () => new Date(2026, 6, 1, 0, 0, 0),
    });
    ...
    slotNames.length = 0;
    await timedSendTui(api, {
      readText: async () => '{"start":"01:30","end":"09:30","display":{"promptRight":false,"appBottom":false}}',
      readStatus: async () => waiting,
      now: () => new Date(2026, 6, 1, 0, 0, 0),
    });
  ...
  test("slot implementation is sidebar-based, reactive, and wraps text nodes", async () => {
    const source = await readFile(join(process.cwd(), "src", "tui.tsx"), "utf8");
